...
function renderSetupControls() {
  ...
  const _xl  = document.getElementById('psExtraAlignLabel'); if (_xl)  _xl.value  = st.extraAlignLabel || 'center';
  const _xq2 = document.getElementById('psExtraAlignQty');   if (_xq2) _xq2.value = st.extraAlignQty   || 'center';
  const _xb  = document.getElementById('psExtraBold');       if (_xb)  _xb.checked = st.extraBold !== false;
  ...
  setV('psOrient',      st.orient      || 'portrait');
...
function saveSetup() {
  ...
  [['RowNo','rowNo'],['Name','name'],['Code','code'],['Unit','unit'],['Sign','sign']].forEach(([sfx,k]) => {
    const a = document.getElementById('psCA'+sfx); if (a && a.value) st.colAlign[k]  = a.value;
  ...
  if (_xq) st.extraQtyW = Math.max(10, Math.min(80, parseFloat(_xq.value) || 40));
  const _xp = document.getElementById('psExtraAlignPage');
  if (_xp) st.extraAlignPage = _xp.value || 'center';
  const _xl = document.getElementById('psExtraAlignLabel');
  if (_xl) st.extraAlignLabel = _xl.value || 'center';
  const _xq2 = document.getElementById('psExtraAlignQty');
  if (_xq2) st.extraAlignQty = _xq2.value || 'center';
  const _xb = document.getElementById('psExtraBold');
  if (_xb) st.extraBold = _xb.checked;
  ...
  else if (vBg != null && vBg !== '#ffffff') st.headBg = vBg;
  const _ah = document.getElementById('psAlignHeader');
